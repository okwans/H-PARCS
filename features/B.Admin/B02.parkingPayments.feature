...
Feature: 주차장_결재내역_확인
  ...
  @B02_01_관리자_주차장_운영정보_확인
  Scenario Outline: 주차장 관리 - 입출차 결재 내역 확인
    Given B01.01 - API Test를 위한 서버가 준비되어 있고 필요한 access Token 데이터를 전달 받았다.
    When B01.01 - API 동작을 통해 특정 차량 "<uid>", "<plateNumber>", "<userName>", "<company>", "<startDay>", "<endDay>"를 이용하여 사전할인 차량 등록을 수행한다.
    Then B01.01 - 전달 받은 data에는 "<plateNumber>", "<payment>", "<entryTime>", "<exitTime>"등의 차량 정보가 포함되어야 한다.
    Examples: API_Data

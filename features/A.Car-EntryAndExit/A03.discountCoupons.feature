...
Feature: 차량_입출차_동작_확인-할인권차량
  ...
  Scenario Outline: 차량 입출차 동작 확인(할인권 차량)
    ...
    When A03.01 - 차량 입고시간은 현재시간 기준 "<inGateDate>"분 전에 입차되는 형태로 "<plateNumber>" 차량을 API를 통해 설정한다.
    And A03.01 - 해당 차량 모두 할인권 "<coupon_cash>"과 "<coupon_time>"을 부여한다.
    Then A03.01 - 해당 차량 출차시 주차시간이 회차시간 미만일 경우 요금 정산은 이뤄지지 않고 회차시간 이상일 경우 over되는 시간 만큼 할인 금액을 제외한 요금 "<bill>"원을 결재해야 한다.
    Examples: API_Data
    | plateNumber | inGateDate | coupon_cash | coupon_time | bill  |
    | 10하9088    | 25         | 0           | 120         | 0     |
    | 20하9099    | 5          | 1000        | 30          | 0     |
    | 30하1010    | 19         | 0           | 30          | 0     |
  ...
  Scenario Outline: 차량 입출차 동작 확인(할인권 차량)
    ...
    When A03.02 - 차량 입고시간은 현재시간 기준 "<inGateDate>"분 전에 입차되는 형태로 "<plateNumber>" 차량을 API를 통해 설정한다.
    And A03.02 - 해당 차량 모두 할인권 "<coupon_cash>"과 "<coupon_time>"을 부여한다.
    And A03.02 - 출차 동작전 사전 정산을 통해 주어진 coupon과 함께 주차 요금을 납부 완료 한다.
    Then A03.02 - 사전 정산 후, "<overTime>"분 후에, 차량 출차시 사전 정산 시간 내/외 여부에 따라 요금 "<bill>"원을 결재해야 한다.
    Examples: API_Data
    | plateNumber | inGateDate | coupon_cash | coupon_time | overTime | bill  |
